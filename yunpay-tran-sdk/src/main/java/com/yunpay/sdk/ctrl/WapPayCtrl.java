...
public class WapPayCtrl extends BaseCtrl{
	...
	public void recvAuthSend(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		Map<String,String> reqParamMap = this.getRequestParams(request);
		Log.debug("微信授权码回调接收参数：" + reqParamMap);
	...
	public Object doWechatwap(HttpServletRequest request, HttpServletResponse response){
		Map<String,String> reqParamMap = this.getRequestParams(request);
		String[] wapParam = {"total_fee","merchant_num","user_order_no","auth_code"};
