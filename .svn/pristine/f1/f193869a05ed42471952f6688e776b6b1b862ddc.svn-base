...
<%-- 			<p>
				<label class="control-label x120">代理商校验码：</label>
				<input type="text" name="agentKey" class="required" data-rule="required length[32]" value="${agent.agentKey}" maxlength="90" size="30" />				
			</p>
			<p>
				<label class="control-label x120">代理商清算方式：</label>
	            <select name="agentsetType" data-toggle="selectpicker">
	            	<c:if test="${agent.agentsetType ==1}">
	                 <option value="1" selected="selected">我方清算</option>
	                 <option value="2">代理清算</option>
	             </c:if>
             <c:if test="${agent.agentsetType ==2}">
                 <option value="1" >我方清算</option>
                 <option value="2"selected="selected">代理清算</option>
             </c:if>
            </select>     
		</p> --%>
		</div>
